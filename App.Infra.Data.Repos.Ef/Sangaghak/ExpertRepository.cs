...
namespace App.Infra.Data.Repos.Ef.Sangaghak
{
    public class ExpertRepository : IExpertRepository
    {
        ...
        {
            return await _appDbContext.Experts.AsNoTracking().ToListAsync();
        }
        ...
        {
            return await _appDbContext.Experts.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Expert> GetExpertByNameAsync(string name)
        {
            return await _appDbContext.Experts.AsNoTracking().FirstOrDefaultAsync(x => x.UserName == name);
        }
        ...
        {
            var Expert=await _appDbContext.Experts.AsNoTracking().FirstOrDefaultAsync(x => x.Email == Email && x.Password==Password);
            if (Expert != null)
            {
                return true;
            }
